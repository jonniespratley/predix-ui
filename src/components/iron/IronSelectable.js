...
export default class IronSelectable extends IronMultiSelectable {
	...
	selectPrevious() {
		...
		let index = (Number(this._valueToIndex(this.selected)) - 1 + length) % length;
	...
	_valueForItem(item) {
		...
		let propValue = item.props[this.attrForSelected];
	...
	_valueForIndex(index) {
		...
		let propValue = this.items[this.attrForSelected];
