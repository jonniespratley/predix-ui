...
describe('Dropdown', () => {
  ...
  test('matches snapshot', () => {
    const tree = renderer
      .create(
        <Dropdown displayValue="My Val" buttonStyle="default" items={items} />
      )
      .toJSON();
  ...
  test('should clear selected when px-utl-close is clicked', () => {
    const wrapper = mount(<Dropdown items={items} selected={1} />);
  ...
  test('should trigger onChange when item is changed', () => {
    let spy = sinon.spy();
    const wrapper = shallow(<Dropdown items={items} onChange={spy} />);
    expect(wrapper.find('.px-button')).toHaveLength(1);
    expect(wrapper.find('.px-dropdown__option')).toHaveLength(items.length);
